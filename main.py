...
grid_center = None; last_report_date = date.today()
last_rebalance = 0
    ...
    try:
        for p in client.fetch_positions([symbol]):
            if p.get("contracts", 0) > 0:
                ...
                client.create_order(symbol, "market", side, p["contracts"],
...
def rebalance_grid():
    ...
    global current_trend, trend_confirmation, position_open_time, trail_stop_price, grid_center
        ...
        if int(now/3600) != int(last_rebalance/3600):
            ...
            if confirmed_trend and ema21_2days(client):
                if current_trend=="buy":
                    upper_pct = ASYMMETRY_RANGE
                    lower_pct = 5.0
                else:
                    upper_pct = 5.0
                    lower_pct = ASYMMETRY_RANGE
                place_grid_orders(client, capital=GRID_CAPITAL*margin_ratio, upper_pct=upper_pct, lower_pct=lower_pct)
...
if __name__ == "__main__":
    ...
    send_telegram("🟢 Bot started (2025-12-08, all 10 opts ON)")
    while True:
        try:
            rebalance_grid()
        except Exception as e:
